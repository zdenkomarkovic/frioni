      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-10">
          {plans.map((plan) => (
            <div
              key={plan.title}
              className="flex items-center gap-5 p-5 rounded-2xl bg-[#0c1220] border border-white/8 hover:border-white/20 transition-all"
            >
              <div className="w-14 h-14 relative flex-shrink-0">
                <Image src={plan.icon} alt={plan.title} fill className="object-contain" />
              </div>
              <div>
                <p className="text-white font-bold text-base mb-1">{plan.title}</p>
        ...
        <p className="text-gray-600 text-xs mt-6">
          Cene su okvirne. Finalna cena se dogovara na licu mesta.
        </p>
      </div>
    </section>
  );
